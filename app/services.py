...
# Cache for 1 hour with a maximum size of 1000 entries
...
def import_data():
    ...
    with app.app_context():
        ...
        timezones_csv = pd.read_csv('data/bq-results-20230125-202210-1674678181880.csv', chunksize=batch_size)
        ...
        for business_hours_df in business_hours_csv:
            for i, row in business_hours_df.iterrows():
                start_time = pd.to_datetime(row['start_time_local'], format='mixed').time()
                end_time = pd.to_datetime(row['end_time_local'], format='mixed').time()
...
# Implementing the threading and caching for concurrency and fast access to in-memory data while also
# implementing for the dynamic database update at every hour
#-------------------------------------------------------------
def start_import_data_thread():
    t = threading.Thread(target=run_import_data)
    t.start()



def refresh_cache(db_lock):
    with db_lock:
        while True:
            import_data()  # Refresh the data in cache
